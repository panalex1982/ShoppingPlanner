...
import com.bue.shoppingplanner.free.model.Buys;
...
public class BoughtController {
	...
	/**
	 * Adds entry to bought table and also on the foreign keys tables if the
	 * records does not exist.
	 * 
	 * @param persistType
	 *            if it is 0 then it persist an bought, if it is 1 it persists a
	 *            list.
	 * @return
	...
	public int persistBought(int persistType) throws NullPointerException {
		...
		for (ShoppingListElementHelper element : products) {
			if (element.isChecked()) {
				...
				barcode = addCommercialProduct(barcode, productName,
						element.getBrand());
	...
	/**
	 * Returns an ArrayList with ShoppingListElementHelper elements, that
	 * contained the information of the shopping list of the parameter listName.
	 * 
	 * @param listName
	 * @return
	 */
	public ArrayList<ShoppingListElementHelper> getSavedShoppingList(
			String listName) {
		...
		for (Buys item : Buys.getShoppingListItems(db, listName)) {
			...
			CommercialProduct cmProduct = CommercialProduct
					.getCommercialProduct(db, product.getBarcode());
			...
			ProductKind kind = ProductKind
					.getProductKind(db, product.getKind());
	...
	public ArrayList<String[]> getUserSpendingByProduct(String userName,
	...
	public ArrayList<CommercialProduct> getCommercialProductByProduct(
			String productName) {
		return CommercialProduct.getCommercialProductByProduct(db, productName);
	}

	public ArrayList<String> getCommercialProductNamesByProduct(
			String productName) {
		ArrayList<String> names = new ArrayList<String>();
		ArrayList<CommercialProduct> cProducts = CommercialProduct
				.getCommercialProductByProduct(db, productName);
	...
	/**
	 * Returns double array with 2 items: double[0] is the price double[1] is
	 * the vat rate
	 * 
	 * @param productId
	 * @return
	 ...
	 * @return
	 */
	public ArrayList<String[]> getFilteredProductSpending(String fromDate,
			String toDate, ArrayList<String> users, ArrayList<String> kinds,
			ArrayList<String> shops, ArrayList<String> brands) {
		return Buys.getFilteredProductSpending(db, fromDate, toDate, users,
				kinds, shops, brands);
	}
	...
	public Collection<? extends String[]> getFilteredUserSpending(
		...
		return Buys.getFilteredUserSpending(db, fromDate, toDate, products,
				kinds, shops, brands);
	}
	...
	public Collection<? extends String[]> getFilteredShopSpending(
			...
			ArrayList<String> brands) {
		return Buys.getFilteredShopSpending(db, fromDate, toDate, products,
				kinds, users, brands);
	}
	...
	public Collection<? extends String[]> getFilteredKindSpending(
			...
			ArrayList<String> brands) {
		return Buys.getFilteredKindSpending(db, fromDate, toDate, products,
				shops, users, brands);
	}
	...
	public Collection<? extends String> getAllBrands() {
		return CommercialProduct.getAllCommercialProductBrandName(db);
	}
	...
	/**
	 * Return String array with 3 elements. The 1st is the product description
	 * or company name, the 2nd is the commercial name of product(it is not used
	 * for easier product input and the 3rd is the product which describes the
	 * barcode.
	 * 
	 * If the 1st element of the return array is "0" then Commercial Product
	 * does not exist.
	 * 
	 * @param barcode
	 * @return
	...
	public String[] getCommerialProduct(String barcode) {
		...
		CommercialProduct cProduct = CommercialProduct.getCommercialProduct(db,
				barcode);
		if (!cProduct.isNull()) {
	...
	public ArrayList<String> getAllProductNamesOfKind(String kindName) {
		return (ArrayList<String>) Product.getAllProductNamesOfKind(db,
				kindName);
	}
	...
	private String addCommercialProduct(String barcode, String commercialName,
			String brand) {
		// Set the commercial
		...
		if (barcode.equalsIgnoreCase(context.getResources().getString(R.string.unknown))) {
			...
			CommercialProduct commercial = new CommercialProduct(
					String.valueOf(unBarcode.getBarcode() - 1), commercialName,
					brand);
			unBarcode.updateUnknownBarcode(db);
			barcode = commercial.addCommercialProduct(db);
			// Has Barcode
		} else {
			CommercialProduct commercial = new CommercialProduct(barcode,
	...
	public void persistProduct(ShoppingListElementHelper element) {
		String barcode = addCommercialProduct(element.getBarcode(),
				element.getProduct(), element.getBrand());
	...
	public String deleteUser(String userName) {
		return User.deleteUser(db, userName);
	}
