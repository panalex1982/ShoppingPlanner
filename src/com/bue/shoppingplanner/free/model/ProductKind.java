...
public class ProductKind {
	...
	public static List<String> getAllProductKindNames(Dbh handler) {
		...
		// Select All Query
		String selectQuery = "SELECT " + Dbh.PRODUCT_KIND_NAME + " FROM "
				+ Dbh.TABLE_PRODUCT_KIND;
	...
	public int getProductKindId(Dbh db) {
		...
		Cursor cursor = readable.query(Dbh.TABLE_PRODUCT_KIND,
				new String[] { Dbh.PRODUCT_KIND_ID, }, Dbh.PRODUCT_KIND_NAME
						+ "=?", new String[] { String.valueOf(name) }, null,
				null, null, null);
		if (cursor != null)
			if (cursor.moveToFirst())
